...
from pyot.core.functional import parse_camelcase, turbo_copy
...
class MatchParticipantStatData(PyotStatic):
    ...
    def items(self):
        ...
        return [Item(id=i) if i > 0 else None for i in self.item_ids]

    @property
    def meraki_items(self):
    ...
    def runes(self):
        ...
        return [Rune(id=i) for i in self.rune_ids]

    @property
    def stat_runes(self):
        from .rune import Rune
        return [Rune(id=i) for i in self.stat_rune_ids]

    @property
    def spells(self):
        from .spell import Spell
        return [Spell(id=i) for i in self.spell_ids]
...
class Match(PyotCore):
    ...
    def __init__(self, id: int = None, tournament_code: str = None, include_timeline: bool = False, platform: str = models.lol.DEFAULT_PLATFORM):
        self.initialize(locals())

    async def get(self, pipeline: str = None, deepcopy: bool = False):
        if not self.include_timeline:
            return await super().get(pipeline, deepcopy)
        get_timeline = asyncio.create_task(Timeline(id=self.id, platform=self.platform).get(pipeline, deepcopy))
        get_match = asyncio.create_task(super().get(pipeline, deepcopy))
        await get_match
...
class Timeline(PyotCore):
    ...
    def __init__(self, id: int = None, platform: str = models.lol.DEFAULT_PLATFORM):
        self.initialize(locals())
...
class MatchHistory(PyotCore):
    ...
    def __init__(self, account_id: str = None, platform: str = models.lol.DEFAULT_PLATFORM):
        self.initialize(locals())
    ...
    def query(self, champion_ids: List[int] = None, queue_ids: List[int] = None, season_ids: List[int] = None, end_time: int = None, begin_time: int = None, end_index: int = None, begin_index: int = None):
        ...
        self._meta.query = parse_camelcase(kargs)
        return self
...
class Matches(PyotCore):
    ...
    def __init__(self, tournament_code: str = None, platform: str = models.lol.DEFAULT_PLATFORM):
        self.initialize(locals())
